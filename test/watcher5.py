...
while (True):
	...
	blurred = cv2.GaussianBlur(gray_one_chan, (1,1), 0)
	edged = cv2.Canny(blurred, 50, 250, 255)


        for i in range(3):
          ret, frame2 = cap.read()
        gray2_one_chan = cv2.cvtColor(frame2, cv2.COLOR_BGR2GRAY)

	blurred2 = cv2.GaussianBlur(gray2_one_chan, (1,1), 0)
	edged2= cv2.Canny(blurred2, 50, 250, 255)
	
	edged_final = cv2.bitwise_or(edged, edged2)

        edged_final_3_chan = cv2.cvtColor(edged_final, cv2.COLOR_GRAY2RGB)


        cv2.imshow('preview', edged_final_3_chan)
        #cv2.waitKey(0)

        #Now write out just last image (in color) for debug, if desired.
        cv2.imwrite("latest_frame.jpg", edged_final_3_chan)
	...
	if args.dummy:
		...
		resized = cv2.resize(edged_final_3_chan, (320,240))
		#print frame.shape
		#print gray.shape
		#print edged.shape
		#print resized.shape
